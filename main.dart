...
void crearVuelo() {
  ...
  vuelos.add(Vuelo(numeroVuelo, destino, origen, fechaVuelo));
}

void listarVuelosDelDia() {
  DateTime hoy = DateTime.now();
  List<Vuelo> vuelosDelDia = vuelos
      .where((vuelo) =>
          vuelo.fechaVuelo.year == hoy.year &&
          vuelo.fechaVuelo.year == hoy.year &&
          vuelo.fechaVuelo.month == hoy.month &&
          vuelo.fechaVuelo.day == hoy.day)
      .toList();
  ...
  if (vuelosDelDia.isEmpty) {
    ...
    for (var vuelo in vuelosDelDia) {
      print(
          'Código: ${vuelo.numeroVuelo}, Destino: ${vuelo.destino}, Fecha: ${vuelo.fechaVuelo.toIso8601String()}');
    }
  }
}

void agregarPasajeroAVuelo() {
  stdout.write('Ingrese el código del vuelo al que desea agregar un pasajero: ');
  int codigo = int.parse(stdin.readLineSync()!);

  try {
    Vuelo? vuelo = vuelos.firstWhere((vuelo) => vuelo.numeroVuelo == codigo);
  ...
  if (vuelo != null) {
    if (vuelo.pasajeros.isEmpty) {
      ...
      for (var pasajero in vuelo.pasajeros) {
        print('nombre: ${pasajero.nombre}, apellido: ${pasajero.apellido}, email: ${pasajero.email}');
      }
    }
  } else {
    print('Vuelo no encontrado.');
